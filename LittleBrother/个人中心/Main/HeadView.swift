...
class HeadView: UIView {
    ...
    func fillContents(_ content: Any?) {
        ...
        let headHolder: UIImage
        if let img = userDefault.getCustomObj(for: kHeadImageKey) {
            headHolder = img as! UIImage
        } else {
            headHolder = UIImage(named: "headPlaceholder")!
        }
        let url = Router.getAvatarURL(person!.id)
        headImageView?.sd_setImage(with: url, placeholderImage: headHolder, options: [.retryFailed, .delayPlaceholder, .refreshCached]) { img, _, _, _ in
            if img != nil {
                userDefault.saveCustomObj(img!, key: kHeadImageKey)
            }
        }
    }
